// server.js - B|ngo Casino (solo asignación individual)
...
function upsertUser(email, cards){
  return new Promise((res,rej)=>{
    ...
    db.run(
      `INSERT INTO users(email,cards,connected,last_seen)
       VALUES(?,?,?,?)
       ON CONFLICT(email)
       DO UPDATE SET cards=excluded.cards, connected=1, last_seen=?`,
      [email, json, 1, ts, ts],
      err => err ? rej(err) : res()
    );
  });
...
function setConnected(email, state){
  return new Promise((res,rej)=>{
    db.run("UPDATE users SET connected=?, last_seen=? WHERE email=?",
      [state?1:0, Date.now(), email],
      err => err?rej(err):res()
    );
  });
}
...
/* ========================
    ANALIZAR CARTÓN
=========================== */
function analyzeCard(card, drawn){
  ...
  for(let r=0;r<5;r++){
    const row=[];
    for(let c=0;c<5;c++){
      row.push(card[cols[c]][r]);
    }
    grid.push(row);
  }

  const marked = grid.map(row =>
    row.map(v => v==="FREE" || drawn.has(Number(v)))
  );

  let terna=false, quintina=false;

  for(let r=0;r<5;r++){
    const x = marked[r].filter(Boolean).length;
    if(x>=3) terna=true;
    if(x===5) quintina=true;
  }
  ...
  for(let r=0;r<5;r++){
    for(let c=0;c<5;c++){
      if(grid[r][c] !== "FREE" && !drawn.has(Number(grid[r][c]))) all=false;
    }
  }

  return {terna, quintina, bingo: all};
}
...
function broadcastState(){
  db.all("SELECT email FROM pending ORDER BY ts DESC", (e,pend=[])=>{
    const pending = pend.map(x=>x.email);

    db.all("SELECT email,cards,connected FROM users", (e,rows=[])=>{
      const players = rows.map(u => {
        let cards=[];
        try { cards = JSON.parse(u.cards)||[]; } catch{}
        return {
          email: u.email,
          cards,
...
io.on("connection", socket=>{
  ...
  socket.on("identify", async email=>{
    ...
    if(user && user.cards){
      try { socket.emit("cardsAssigned", JSON.parse(user.cards)); } catch {}
    }
  ...
  socket.on("adminAssignOne", async ({email, count})=>{
    ...
    if(sid) io.to(sid).emit("cardsAssigned", cards);
  ...
  socket.on("adminDrawNumber", ()=>{
    ...
    db.all("SELECT email,cards FROM users", (e,rows)=>{
      if(!rows) return;
      ...
      rows.forEach(r=>{
        ...
        cards.forEach((card,idx)=>{
          ...
          if(a.terna || a.quintina || a.bingo){
            const notif = {
              email: r.email, cardIndex: idx,
              terna:a.terna, quintina:a.quintina, bingo:a.bingo
            };
  ...
  socket.on("disconnect", async ()=>{
    ...
    if(email){
      ...
      await setConnected(email,false).catch(()=>{});
    }
    broadcastState();
  });
